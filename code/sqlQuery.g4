...
ID_LITERAL:
	'*'
	| ('@' | '_' | LETTER) (LETTER | DEC_DIGIT | '_')*;
REVERSE_QUOTE_ID: '`' ~'`'+ '`';
DECIMAL_LITERAL: DEC_DIGIT+;

// 2.2 parser - 语法 解析规则(Parser rule)名小写开头,后面跟字母、数字、下划线 - 语法
...
tableSources: tableName (',' tableName)*;
...
groupByCaluse: GROUP BY groupByItem (',' groupByItem)*;
havingCaluse: HAVING logicExpression;
...
orderByExpression: fullColumnName order = (ASC | DESC)?;

groupByItem: fullColumnName order = (ASC | DESC)?;
...
value: uid | textLiteral | decimalLiteral;
...
functionCall: aggregateWindowedFunction # aggregateFunctionCall;

aggregateWindowedFunction: (AVG | MAX | MIN | SUM) '(' functionArg ')'
	| COUNT '(' (starArg = '*' | functionArg?) ')'
	| COUNT '(' aggregator = DISTINCT functionArgs ')';

functionArg: column_name;

functionArgs: column_name (',' column_name)*;
